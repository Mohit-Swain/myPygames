...
def valid_space(shape, grid):  # doubts
    accepted_pause = [[(j, i) for j in range(10) if grid[i][j] == (0, 0, 0)] for i in range(20)]
    accepted_pause = [j for sub in accepted_pause for j in sub]

    formatted = convert_shape_format(shape)
    for pos in formatted:
        if pos[0] >= 10 or pos[0] < 0:
            return False
        if pos not in accepted_pause:
            if pos[1] > -1:
                return False
    return True
...
def clear_rows(grid, locked):
    ...
    return inc * 10  # increase in score
...
def draw_window(surface, grid, score=0):
    ...
    text1 = font1.render('Tetris', 1, (255, 255, 255))
    surface.blit(text1, (top_left_x + play_width // 2 - (text1.get_width() // 2), 30))

    font = pygame.font.SysFont('comicsansms', 30)
    text = font.render('Score: ' + str(score), 1, (255, 255, 255))
    sx = top_left_x + play_width + 50
    sy = top_left_y + play_height // 2 - 100
    surface.blit(text, (sx + 10, sy + 180))
...
def main(surface):
    ...
    while run:
        ...
        if change_piece:
            ...
            next_piece = get_shape()
            change_piece = False
            # if the pieces are static then clear row
            score += clear_rows(grid, locked_position)
